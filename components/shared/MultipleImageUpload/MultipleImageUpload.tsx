  ...
  return (
    <div className="">
      ...
      {showWarning && (
        <div className="mb-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg flex items-center gap-2 text-yellow-700">
          <FiAlertCircle className="w-5 h-5" />
          <span>Maximum {maxFiles} images allowed</span>
        </div>
      )}
          ...
          >
            {file.status === 'uploading' ? (
              ...
              <>
                ...
                <button
                  onClick={() => removeFile(file.id)}
                  className="absolute top-6 right-6 p-1 rounded-full bg-red-500 text-white opacity-0 group-hover:opacity-100 transition-opacity"
                >
                  <RiDeleteBin6Line className="w-4 h-4 transform" />
                </button>
              </>
